...
async function getAICorrectedSubtitle(infoHash, language) {
    ...
    // Third fallback: subdl.com (unofficial, HTML scrape)
    if (!subFileUrl) {
        try {
            let keywords = infoHash;
            if (typeof infoHash === 'object' && infoHash !== null) {
            ...
            // Subdl search URL (Turkish only, if possible)
            const subdlUrl = `https://subdl.com/search.php?search=${encodeURIComponent(keywords)}`;
            const html = await fetch(subdlUrl).then(r => r.text());
            // Try to find Turkish .srt link
            const trMatch = html.match(/href="(\/tr\/subtitle\/\d+\/.*?)"/);
            const enMatch = html.match(/href="(\/en\/subtitle\/\d+\/.*?)"/);
            if (trMatch && trMatch[1]) {
                subFileUrl = `https://subdl.com${trMatch[1]}`;
            } else if (enMatch && enMatch[1]) {
                subFileUrl = `https://subdl.com${enMatch[1]}`;
                console.warn('No Turkish subtitle found on subdl.com, falling back to English.');
            } else {
                console.warn('No .srt subtitle link found on subdl.com for keywords:', keywords);
            }
        } catch (e) {
            console.error('Subdl fallback failed:', e.message);
        }
    }
...
async function getSubtitleUrlsForStremio(infoHash, preferredLanguages = ['en']) {
    ...
    // If infoHash is a TMDb ID, convert to IMDb ID using TMDb API
    let idToUse = infoHash;
    if (typeof infoHash === 'string' && infoHash.startsWith('tmdb:')) {
        const tmdbId = infoHash.split(':')[1];
        ...
        if (tmdbApiKey) {
            try {
                ...
                console.error('TMDb to IMDb conversion failed:', e.message);
            }
        } else {
            console.warn('TMDB_API_KEY not set, cannot convert TMDb ID to IMDb ID.');
        }
    }
    const now = Date.now();
    if (now - config.opensubtitlesUsage.lastReset > 24 * 60 * 60 * 1000) {
    ...
    // Always add AI-corrected subtitle option for each preferred language
    let aiCorrectedSubs = (preferredLanguages || ['en']).map(lang => ({
        lang,
        url: `/ai-corrected-subtitle/${typeof idToUse === 'object' && idToUse !== null ? (idToUse.infoHash || idToUse.hash || '') : idToUse}/${lang}`,
        id: `ai-corrected-${lang}`,
        behaviorHints: { directDownload: true }
    }));

    let hashToUse = idToUse;
    let title = '', year = '', imdbId = '', tmdbId = null;
    if (typeof idToUse === 'object' && idToUse !== null) {
        hashToUse = idToUse.infoHash || idToUse.hash || '';
        title = idToUse.title || '';
        year = idToUse.year || '';
        imdbId = idToUse.imdb_id || idToUse.imdbId || '';
    } else if (typeof idToUse === 'string' && idToUse.startsWith('tt')) {
        imdbId = idToUse;
    } else if (typeof idToUse === 'string' && idToUse.startsWith('tmdb:')) {
        tmdbId = idToUse.split(':')[1];
    } else if (typeof idToUse === 'string' && idToUse.length > 0) {
        // GuessIt fallback: try to extract metadata from filename using guessit.io API
        try {
            const guessitRes = await fetch(`https://guessit.io/api/v1/guess?filename=${encodeURIComponent(idToUse)}`);
            if (guessitRes.ok) {
                const guessitData = await guessitRes.json();
                title = guessitData.title || '';
                year = guessitData.year ? String(guessitData.year) : '';
                if (guessitData.imdb_id) imdbId = guessitData.imdb_id;
                // Optionally: season/episode for TV
                if (guessitData.season) title += ` S${String(guessitData.season).padStart(2,'0')}`;
                if (guessitData.episode) title += `E${String(guessitData.episode).padStart(2,'0')}`;
            }
        } catch (e) {
            console.warn('GuessIt API fallback failed:', e.message);
        }
    }
    ...
    if (!usePodnapisi) {
        ...
        if (imdbId) {
            ...
            // If no IMDb ID, use the TMDb ID as a fallback
                ...
                if (data.data && data.data.length > 0) {
                    ...
                    results = data.data
                        ...
                        .map(sub => {
                            ...
                            // Mark perfect match if hash search and this subtitle's moviehash matches
                            let perfectMatch = false;
                            if (isHashSearch && sub.attributes.moviehash && hashToUse && sub.attributes.moviehash === hashToUse) {
                                perfectMatch = true;
                            }
                            return fileUrl ? {
    ...
    // Third fallback: subdl.com (API if available, else HTML scrape)
    if (results.length === 0) {
        try {
            let keywords = infoHash;
            if (typeof infoHash === 'object' && infoHash !== null) {
            ...
            if (config.SUBDL_API_URL) {
                // Example: GET <SUBDL_API_URL>?q=keywords&lang=en
                const apiUrl = `${config.SUBDL_API_URL}?q=${encodeURIComponent(keywords)}&lang=${preferredLanguages[0] || 'en'}`;
                const apiRes = await fetch(apiUrl);
                if (apiRes.ok) {
                    const apiData = await apiRes.json();
                    if (Array.isArray(apiData.subtitles) && apiData.subtitles.length > 0) {
                        results = apiData.subtitles.map((sub, i) => ({
                            lang: sub.lang || preferredLanguages[0] || 'en',
                            url: sub.url,
                            id: `subdl-api-${sub.lang || preferredLanguages[0] || 'en'}-${i}`,
                            behaviorHints: { directDownload: true }
                        }));
                    }
                }
            }
            // Fallback to HTML scrape if no API or no results
            if (results.length === 0) {
                const subdlUrl = `https://subdl.com/search.php?search=${encodeURIComponent(keywords)}`;
                const html = await fetch(subdlUrl).then(r => r.text());
                // Find all .srt links (very basic, English only)
                const matches = [...html.matchAll(/href="(\/en\/subtitle\/\d+\/.*?)"/g)];
                if (matches.length > 0) {
                    results = matches.map((m, i) => ({
                        lang: 'en',
                        url: `https://subdl.com${m[1]}`,
                        id: `subdl-en-${i}`,
                        behaviorHints: { directDownload: true }
                    }));
                }
            ...
            console.error('Subdl fallback for Stremio failed:', e.message);
        }
    }
    // Prepend AI-corrected subs to the results (so they show up first in Stremio)
    return [...aiCorrectedSubs, ...results];
}
