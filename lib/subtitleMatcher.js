...
// Advanced AI Subtitle Correction with 12-Step Professional Analysis
async function getAICorrectedSubtitle(subtitleContent, movieTitle, year, season, episode, language = 'tr') {
    try {
        console.log(`[subtitleMatcher] Starting AI correction for ${movieTitle} (${year})`);
        
        // Check if AI is enabled
        if (!process.env.AI_ENABLED || process.env.AI_ENABLED !== 'true') {
            console.log(`[subtitleMatcher] AI correction disabled, returning original subtitle`);
            return subtitleContent;
        }
        
        // Define the comprehensive 12-step AI prompt
        const aiPrompt = `
You are a professional subtitle timing and content specialist. Analyze and improve the following subtitle file using this comprehensive 12-step process:

**CRITICAL ANALYSIS STEPS:**

1. **TIMING SYNCHRONIZATION ANALYSIS**
   - Analyze timing patterns for consistency
   - Identify potential desynchronization issues
   - Check for proper reading speed (typically 150-200 words per minute)
   - Verify subtitle duration matches speech patterns

2. **SUBTITLE DURATION OPTIMIZATION**
   - Ensure minimum duration of 1 second per subtitle
   - Maximum duration should not exceed 7 seconds
   - Adjust timing for optimal reading comprehension
   - Balance between readability and natural speech flow

3. **TEXT SEGMENTATION REVIEW**
   - Check for proper line breaks at natural speech pauses
   - Ensure no more than 2 lines per subtitle
   - Optimize text distribution for balanced reading
   - Maintain semantic coherence across line breaks

4. **LANGUAGE AND GRAMMAR CORRECTION**
...
6. **TECHNICAL FORMAT VALIDATION**
   - Verify SRT format compliance
   - Check subtitle numbering sequence
   - Ensure proper timestamp format
   - Validate character encoding compatibility

7. **READING SPEED OPTIMIZATION**
   - Calculate and optimize reading speed for each subtitle
   - Ensure comfortable reading pace for average viewers
   - Adjust timing for complex or technical dialogue
   - Consider subtitle length vs. available time

8. **DIALOGUE SYNCHRONIZATION**
   - Align subtitles with actual speech patterns
   - Ensure proper timing for question-answer sequences
   - Maintain natural conversation flow
   - Consider speaker changes and overlapping dialogue

9. **CONSISTENCY VERIFICATION**
   - Check for consistent terminology usage
   - Ensure uniform formatting throughout
   - Verify consistent character name translations
   - Maintain style consistency across episodes/scenes

10. **ACCESSIBILITY ENHANCEMENT**
    - Ensure subtitles are accessible to hearing-impaired viewers
    - Add sound effect descriptions where appropriate
    - Optimize contrast and readability considerations
    - Consider subtitle positioning for visual elements

11. **QUALITY ASSURANCE REVIEW**
    - Perform final quality check for errors
    - Verify subtitle completeness
    - Check for missing or duplicated subtitles
    - Ensure smooth viewing experience

12. **PROFESSIONAL POLISH**
    - Apply final professional touches
    - Ensure subtitles meet broadcast standards
    - Optimize for different viewing platforms
    - Deliver cinema-quality subtitle experience

**SUBTITLE CONTENT TO PROCESS:**
Title: ${movieTitle}
Year: ${year}
...
- Apply ALL 12 steps systematically
- Maintain original timing structure where possible
- Improve only where necessary for quality enhancement
- Preserve original meaning and context
- Output in proper SRT format
- Ensure professional broadcast quality

**OUTPUT FORMAT:**
Return the improved subtitle content in proper SRT format with:
- Sequential numbering starting from 1
- Proper timestamp format (HH:MM:SS,mmm --> HH:MM:SS,mmm)
- Clean, professional text content
- Optimized timing and duration
- Enhanced readability and accuracy

Please process this subtitle file through all 12 professional analysis steps and return the enhanced version.
`;
        ...
        if (process.env.GEMINI_API_KEY && !aiResponse) {
            try {
                console.log(`[subtitleMatcher] Attempting AI correction with Gemini`);
                aiResponse = await callGeminiAI(aiPrompt);
            } catch (e) {
                console.warn(`[subtitleMatcher] Gemini AI failed: ${e.message}`);
            }
        }
        
        // Try OpenAI (if configured and Gemini failed)
        if (process.env.OPENAI_API_KEY && !aiResponse) {
            try {
                console.log(`[subtitleMatcher] Attempting AI correction with OpenAI`);
                aiResponse = await callOpenAI(aiPrompt);
            } catch (e) {
                console.warn(`[subtitleMatcher] OpenAI failed: ${e.message}`);
            }
        }
        
        // Try Claude (if configured and others failed)
        if (process.env.ANTHROPIC_API_KEY && !aiResponse) {
            try {
                console.log(`[subtitleMatcher] Attempting AI correction with Claude`);
                aiResponse = await callClaudeAI(aiPrompt);
            } catch (e) {
                console.warn(`[subtitleMatcher] Claude AI failed: ${e.message}`);
            }
        }
        
        if (aiResponse) {
            console.log(`[subtitleMatcher] AI correction completed successfully`);
            return aiResponse;
        } else {
            console.log(`[subtitleMatcher] No AI provider available, returning original subtitle`);
            return subtitleContent;
        }
        
    } catch (e) {
        console.error(`[subtitleMatcher] Error in AI correction:`, e);
        return subtitleContent;
    }
}
...
async function callGeminiAI(prompt) {
    const response = await robustFetch('https://generativelanguage.googleapis.com/v1beta/models/gemini-pro:generateContent?key=' + process.env.GEMINI_API_KEY, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
        },
...
// Main function to get subtitle URLs for Stremio
async function getSubtitleUrlsForStremio(imdbId, type, season, episode, language = 'tr') {
    try {
        console.log(`[subtitleMatcher] Starting subtitle search for ${imdbId} (${type})`);
        ...
        results.forEach((result, index) => {
            if (result.status === 'fulfilled' && result.value) {
                ...
                console.warn(`[subtitleMatcher] Provider ${index} failed: ${result.reason}`);
            }
        });
        ...
        console.log(`[subtitleMatcher] Found ${subtitles.length} subtitles for ${imdbId}`);
...
const getAiEnhancementStatus = () => aiEnhancementStatus;
