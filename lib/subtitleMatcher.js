...
async function fetchSubdlSubtitle(videoId, infoHash) {
    try {
        ...
        // Build search request parameters - Enhanced approach from a4kSubtitles
        const params = {
            api_key: subdlApiKey,
            languages: 'tr',
            type: 'movie',
        ...
        // PRIORITY 1: Hash-based search if available
        if (infoHash) {
            console.log('[subtitleMatcher] SubDL: Using hash-based search');
            params.hash = infoHash;
        } else if (imdbId && imdbId.startsWith('tt')) {
            console.log('[subtitleMatcher] SubDL: Using IMDb ID search');
            params.imdb_id = imdbId;
            // Try to get year from IMDb if possible
            try {
                const movieData = await getMovieMetadata(imdbId);
                if (movieData && movieData.year) {
                    params.year = movieData.year;
                }
            } catch (e) {
                console.log('[subtitleMatcher] Failed to get movie metadata for year:', e);
            }
        } else {
            console.log('[subtitleMatcher] SubDL: No valid search parameters available');
            return null;
        }
        
        const url = `https://api.subdl.com/api/v1/subtitles?${new URLSearchParams(params)}`;
        ...
        if (!res.ok) {
            ...
            console.log('[subtitleMatcher] SubDL API error:', res.status, res.statusText, errorText);
            return null;
        }
        
        const data = await res.json();
        console.log('[subtitleMatcher] SubDL API response status:', data.status);
        
        if (!data.status) {
            console.log('[subtitleMatcher] SubDL API returned error:', data.message || 'Unknown error');
            return null;
        }
        ...
        // Enhanced Turkish subtitle filtering with better matching
        const turkishSubs = data.subtitles.filter(sub => {
            ...
            // Check language field and release name for Turkish indicators
            const isTurkish = language === 'tr' || language === 'tur' || language === 'turkish' ||
                             releaseName.includes('turkish') || releaseName.includes('türkçe') || 
                             releaseName.includes('tr.') || releaseName.includes('.tr.');
            
            console.log(`[subtitleMatcher] SubDL subtitle check: lang="${language}", release="${releaseName}", turkish=${isTurkish}`);
            return isTurkish;
        });
        ...
        if (turkishSubs.length > 0) {
            // Enhanced sorting - prioritize by hearing impaired flag, download count, rating
            ...
            console.log('[subtitleMatcher] Selected SubDL subtitle:', JSON.stringify(subtitle, null, 2));
            
            // Build download URL - Enhanced URL construction
...
async function fetchPodnapisiSubtitle(videoId, infoHash) {
    try {
        ...
        // Build search request using JSON API (a4kSubtitles approach)
        const searchParams = {
            keywords: movieData ? movieData.title : imdbId,
            language: ['tr'], // Turkish language
            movie_type: 'movie'
        };
        
        if (movieData && movieData.year) {
            searchParams.year = movieData.year;
        }
        
        const searchUrl = `https://www.podnapisi.net/subtitles/search/advanced?${new URLSearchParams(searchParams)}`;
        ...
        if (contentType && contentType.includes('application/json')) {
            // JSON API response (preferred)
            searchData = await searchRes.json();
            console.log('[subtitleMatcher] Podnapisi JSON response:', JSON.stringify(searchData, null, 2));
            
            if (searchData && searchData.data && searchData.data.length > 0) {
                // Process JSON results using a4kSubtitles approach
                const turkishSubs = searchData.data.filter(result => {
                    const langCode = result.language;
                    return langCode === 'tr' || langCode === 'tur';
                });
                
                if (turkishSubs.length > 0) {
                    // Sort by similarity and custom releases
                    turkishSubs.sort((a, b) => {
                        const aReleases = a.custom_releases || [];
                        const bReleases = b.custom_releases || [];
                        const aHasReleases = aReleases.length > 0;
                        const bHasReleases = bReleases.length > 0;
                        
                        // Prefer subtitles with custom releases
                        if (aHasReleases !== bHasReleases) {
                            return bHasReleases ? 1 : -1;
                        }
                        
                        // Then by download count or rating if available
                        const aDownloads = parseInt(a.downloads || 0);
                        const bDownloads = parseInt(b.downloads || 0);
                        return bDownloads - aDownloads;
                    });
                    
                    const bestSubtitle = turkishSubs[0];
                    const downloadUrl = `https://www.podnapisi.net${bestSubtitle.download}`;
                    
                    console.log('[subtitleMatcher] Selected Podnapisi subtitle (JSON):', bestSubtitle);
                    console.log('[subtitleMatcher] Podnapisi download URL:', downloadUrl);
                    return downloadUrl;
                }
            }
        } else {
            ...
            console.log('[subtitleMatcher] Podnapisi HTML response length:', searchHtml.length);
            
            // Enhanced Turkish subtitle extraction with better quality scoring
            const turkishDownloadUrls = await extractTurkishSubtitlesFromPodnapisi(searchHtml, 'json-fallback');
            
            if (turkishDownloadUrls.length > 0) {
                console.log(`[subtitleMatcher] Found ${turkishDownloadUrls.length} Turkish subtitles via HTML parsing`);
        ...
        console.log('[subtitleMatcher] No Turkish subtitles found on Podnapisi');
        return null;
    ...
    try {
        ...
        try {
            ...
            if (isValidSubtitleContent(plainText)) {
                console.log(`[SubtitleMatcher] Content is already plain text, no decompression needed`);
                return plainText;
            }
        } catch (e) {
        ...
        for (const { name, method } of decompressionMethods) {
            try {
                ...
                if (result && result.length > 10 && isValidSubtitleContent(result)) {
                    console.log(`[SubtitleMatcher] Successfully decompressed using ${name} method`);
                    return result;
                }
                console.log(`[SubtitleMatcher] ${name} method produced invalid subtitle content`);
            } catch (methodError) {
                console.log(`[SubtitleMatcher] ${name} decompression method failed: ${methodError.message}`);
            }
        }
...
// Enhanced subtitle ranking system inspired by a4kSubtitles
function rankSubtitleResult(subtitle, videoId, infoHash) {
    let score = 0;
    const title = (subtitle.title || '').toLowerCase();
    const source = subtitle.source || '';
    ...
    score += sourceScores[source.toLowerCase()] || 50;
    
    // Quality indicators in filename
    const qualityIndicators = {
        'bluray': 20, 'bdrip': 18, 'brrip': 16, 'remux': 25,
        'web-dl': 15, 'webrip': 12, 'webdl': 15,
        'dvdrip': 10, 'dvdscr': 8,
        '4k': 30, '2160p': 28, '1080p': 20, '720p': 15, '480p': 10,
        'x265': 15, 'x264': 12, 'hevc': 15, 'h264': 12,
        'aac': 5, 'ac3': 5, 'dts': 8, 'atmos': 10
    };
    ...
    const releaseGroups = [
        'rarbg', 'yify', 'sparks', 'ganool', 'shaanig', 'pahe',
        'cmrg', 'etrg', 'ettv', 'rartv', 'torrentgalaxy'
    ];
    ...
    // Language specificity (Turkish indicators)
    const turkishIndicators = ['turkish', 'türkçe', 'tr.', '.tr.', 'turkce'];
    ...
    const hiIndicators = ['hi', 'cc', 'sdh', 'hearing.impaired'];
    for (const indicator of hiIndicators) {
        if (title.includes(indicator)) {
            score -= 5;
        }
    }
    ...
    if (infoHash && title.includes(infoHash.substring(0, 8))) {
        score += 30; // High bonus for hash match
    }
    
    if (videoId && title.includes(videoId.replace('tt', ''))) {
        score += 15; // Bonus for IMDb ID match
    }
    
    return Math.max(0, score);
}

// Enhanced subtitle filtering and sorting
function filterAndSortSubtitles(subtitles, videoId, infoHash, maxResults = 10) {
    if (!subtitles || subtitles.length === 0) return [];
    
    console.log(`[subtitleMatcher] Filtering and sorting ${subtitles.length} subtitles`);
    
    // Add scores to each subtitle
    const scoredSubtitles = subtitles.map(subtitle => ({
        ...subtitle,
        score: rankSubtitleResult(subtitle, videoId, infoHash)
    }));
    ...
    for (const subtitle of scoredSubtitles) {
        if (!seenUrls.has(subtitle.url)) {
            seenUrls.add(subtitle.url);
            uniqueSubtitles.push(subtitle);
        }
    ...
    console.log(`[subtitleMatcher] After filtering: ${uniqueSubtitles.length} unique subtitles`);
    
    // Return top results
    const topResults = uniqueSubtitles.slice(0, maxResults);
    
    if (topResults.length > 0) {
        console.log(`[subtitleMatcher] Top subtitle scores:`, topResults.map(s => ({
