...
async function getSubtitleUrlsForStremio(videoId, infoHash) {
    try {
        ...
        if (subdlUrl) {
            ...
            // Use direct URL from SubDL instead of processing through AI
            subtitles.push({
                id: `subdl-tr`,
            ...
            console.log(`[SubtitleMatcher] SubDL subtitle added to options.`);
        }
        
        // 2. Try Podnapisi (id only) - temporarily disabled
        console.log(`[SubtitleMatcher] Podnapisi temporarily disabled due to API issues`);
        
        // 3. Always provide OpenSubtitles AI fallback
        ...
        subtitles.push({
            ...
            lang: SUPPORTED_LANGUAGE,
            url: aiSubtitleUrl,
            behaviorHints: { notWebReady: false, aiEnhanced: true, opensubtitles: true }, // Changed to false for desktop compatibility
            name: 'OpenSubtitles AI Turkish'
        });
        console.log(`[SubtitleMatcher] OpenSubtitles AI fallback added.`);
        
        // 4. If no subtitles found, provide a basic fallback
        if (subtitles.length === 0) {
            console.log(`[SubtitleMatcher] No subtitles found, providing basic fallback`);
            ...
            subtitles.push({
                ...
                behaviorHints: { notWebReady: false, fallback: true }, // Changed to false for desktop compatibility
