...
// Enhanced quick subtitle discovery with parallel searching and quality ranking
async function getSubtitleQuickly(videoId, infoHash) {
    try {
        console.log(`[SubtitleMatcher] Starting enhanced parallel subtitle discovery for ${videoId}`);
        
        // Prepare all subtitle sources for parallel execution
        const subtitleSources = [
            {
                name: 'subdl',
                priority: 1,
                hashSupport: true,
                fetcher: () => fetchSubdlSubtitle(videoId, infoHash)
            },
            {
                name: 'opensubtitles',
                priority: 2,
                hashSupport: true,
                fetcher: () => fetchOpenSubtitlesSubtitle(videoId, infoHash)
            },
            {
                name: 'podnapisi',
                priority: 3,
                hashSupport: true,
                fetcher: () => fetchPodnapisiSubtitle(videoId, infoHash)
            }
        ];
        
        // Sort by priority (hash-based sources first if infoHash is available)
        if (infoHash) {
            subtitleSources.sort((a, b) => {
                if (a.hashSupport && !b.hashSupport) return -1;
                if (!a.hashSupport && b.hashSupport) return 1;
                return a.priority - b.priority;
            });
        }
        
        console.log(`[SubtitleMatcher] Executing ${subtitleSources.length} subtitle sources in parallel`);
        
        // Execute all sources in parallel with timeout
        const sourcePromises = subtitleSources.map(async (source) => {
            const startTime = Date.now();
            try {
                console.log(`[SubtitleMatcher] Starting ${source.name} search...`);
                ...
                console.log(`[SubtitleMatcher] ${source.name} completed in ${duration}ms`);
                ...
                if (result) {
                    return {
                        ...
                        duration: duration,
                        hashMatched: !!infoHash && source.hashSupport
                    };
                }
                return null;
            } catch (error) {
                const duration = Date.now() - startTime;
                console.error(`[SubtitleMatcher] ${source.name} failed in ${duration}ms:`, error.message);
                return null;
            }
        });
        ...
        console.log(`[SubtitleMatcher] Found ${successfulResults.length} successful subtitle sources`);
        
        if (successfulResults.length === 0) {
            console.log(`[SubtitleMatcher] No subtitles found from any source`);
            return [];
        }
        
        // Enhanced quality ranking system
        const rankedResults = successfulResults.map(result => {
            let qualityScore = 0;
            
            // Base priority score (lower priority number = higher score)
            qualityScore += (10 - result.priority) * 10;
            ...
            if (result.duration < 2000) qualityScore += 20;
            else if (result.duration < 4000) qualityScore += 10;
            else if (result.duration < 6000) qualityScore += 5;
            
            // Source-specific quality bonuses
            switch (result.source) {
                case 'subdl':
                    qualityScore += 15; // SubDL often has high-quality Turkish subtitles
                    break;
                case 'opensubtitles':
                    qualityScore += 10; // OpenSubtitles is reliable
                    break;
                case 'podnapisi':
                    qualityScore += 8; // Podnapisi has good quality but limited Turkish content
                    break;
            }
        ...
        rankedResults.sort((a, b) => b.qualityScore - a.qualityScore);
        
        console.log(`[SubtitleMatcher] Ranked subtitle sources:`);
        rankedResults.forEach((result, index) => {
            console.log(`[SubtitleMatcher] ${index + 1}. ${result.source} (score: ${result.qualityScore}, duration: ${result.duration}ms, hash: ${result.hashMatched})`);
        });
        
        return rankedResults;
        
    } catch (error) {
        console.error('[SubtitleMatcher] Error in enhanced subtitle discovery:', error);
        return [];
    }
}