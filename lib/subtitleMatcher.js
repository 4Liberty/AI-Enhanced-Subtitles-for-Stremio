...
async function getAICorrectedSubtitle(infoHash, language) {
    ...
    if (!subFileUrl) {
        try {
            ...
            // Subdl search URL (English only, for demo; can be extended)
            ...
            // Very basic scrape: look for first .srt link
            const match = html.match(/href="(\/en\/subtitle\/\d+\/.*?)"/);
            if (match && match[1]) {
                subFileUrl = `https://subdl.com${match[1]}`;
            }
        } catch (e) {
