...
async function getSubtitleUrlsForStremio(imdbId, type, season, episode, language = 'tr', infoHash = null) {
    ...
    try {
        ...
        if (infoHash) {
            ...
            // Process hash-matched results with detailed logging
            for (let i = 0; i < hashMatchedResults.length; i++) {
                ...
                if (result.status === 'fulfilled' && result.value) {
                    try {
                        ...
                        console.warn(`[${requestId}] Hash-matched processing failed for ${provider.name}:`, e.message);
                    }
        ...
        // Process regular subtitle results
        ...
        for (let i = 0; i < regularResults.length; i++) {
            ...
            if (result.status === 'fulfilled' && result.value && Array.isArray(result.value)) {
                ...
                allSubtitleUrls.push(...result.value.map(url => ({ url, provider: provider.name })));
            } else if (result.status === 'rejected') {
        ...
        for (const batch of processingBatches) {
            ...
            const batchPromises = batch.map(async (subtitleInfo, index) => {
                ...
                try {
                    ...
                    console.warn(`[${subtitleId}] Processing failed:`, error.message);
                    return null;
                }
            });
