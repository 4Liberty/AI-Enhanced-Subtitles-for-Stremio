    ...
    const streamCheckPromises = originalStreams.map(async (stream) => {
        if (stream.infoHash) {
            try {
                ...
                const availableLangs = await Promise.race([
                    getAvailableLanguagesForHash(stream.infoHash, userPreferredLanguages),
                ...
                console.error(`Subtitle enrichment failed for stream ${stream.infoHash}:`, err);
                // Return stream as-is on error
            }
        }
        return stream;
    });
