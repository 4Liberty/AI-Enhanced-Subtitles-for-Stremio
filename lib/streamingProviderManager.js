...
class StreamingProviderManager {
  ...
  registerProvider(name, config) {
    ...
    this.providers[name] = {
      ...config,
      ...providerInfo,
      type: name, // Store the provider type
      enabled: true,
      lastError: null,
      errorCount: 0,
      client: null
  ...
  // Create All-Debrid client instance
  createAllDebridClient(token, userIP = null) {
    return new AllDebridClient(token, userIP);
  }
  ...
  async tryProviderStream(providerName, infoHash, magnetLink, options = {}) {
    ...
    try {
      ...
      switch (provider.type) {
        case 'realdebrid':
          const rdClient = this.createRealDebridClient(provider.apiKey, provider.userIP);
          ...
          break;
        case 'alldebrid':
          const adClient = this.createAllDebridClient(provider.apiKey, provider.userIP);
          streamResult = await adClient.createStreamingURL(
            infoHash,
            magnetLink,
            options.filename,
            options.season,
  ...
  async validateProvider(providerName) {
    ...
    try {
      switch (provider.type) {
        case 'realdebrid':
          const rdClient = this.createRealDebridClient(provider.apiKey, provider.userIP);
          const rdUserInfo = await rdClient.getUserInfo();
          return {
            valid: true,
            user: rdUserInfo,
            provider: providerName
        ...
        case 'alldebrid':
          const adClient = this.createAllDebridClient(provider.apiKey, provider.userIP);
          const adUserInfo = await adClient.getAccountInfo();
          return {
            valid: true,
            user: adUserInfo,
            provider: providerName
      ...
      try {
        switch (provider.type) {
          case 'realdebrid':
            const rdClient = this.createRealDebridClient(provider.apiKey, provider.userIP);
            const rdUserInfo = await rdClient.getUserInfo();
            const rdActiveTorrents = await rdClient.getActiveTorrents();
            
            stats[providerName] = {
              type: provider.type,
              user: rdUserInfo.username,
              premium: rdUserInfo.premium,
              expiration: rdUserInfo.expiration,
              activeTorrents: rdActiveTorrents.nb,
              torrentLimit: rdActiveTorrents.limit,
              enabled: provider.enabled,
              errorCount: provider.errorCount
            };
            break;
          case 'alldebrid':
            ...
            stats[providerName] = {
              type: provider.type,
              user: adUserInfo.username,
              premium: adUserInfo.premium,
              expiration: adUserInfo.expiration,
              enabled: provider.enabled,
              errorCount: provider.errorCount
            };
            break;
